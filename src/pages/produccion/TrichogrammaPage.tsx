...
type Confirm = { id: number; tipo: 'produccion' | 'nota'; accion: 'anular' | 'eliminar' }
          ...
          mensaje={
            confirm.accion === 'eliminar'
              ? (confirm.tipo === 'produccion'
                  ? 'Esto borrará este registro de producción de Trichogramma de forma permanente. No se puede deshacer.'
                  : 'Esto borrará esta nota de salida de avispitas de forma permanente. No se puede deshacer.')
              : (confirm.tipo === 'produccion'
                            ...
                            <td className="d-flex gap-1">
                              ...
                              {r.activo ? (
                                <button className="btn btn-sm btn-outline-danger" style={{ fontSize: '.72rem', padding: '2px 8px' }} disabled={anular.isPending}
                                  onClick={() => setConfirm({ id: r.id, tipo: 'produccion', accion: 'anular' })}>Anular</button>
                              ) : (
                                <button className="btn btn-sm btn-danger" style={{ fontSize: '.72rem', padding: '2px 8px' }} disabled={eliminar.isPending} title="Eliminar permanentemente"
                                  onClick={() => setConfirm({ id: r.id, tipo: 'produccion', accion: 'eliminar' })}>🗑</button>
                              )}
                            </td>
                          </tr>
                        ))
                      ...
                      <td className="d-flex gap-1">
                        ...
                        {n.activo ? (
                          <button className="btn btn-sm btn-outline-danger" style={{ fontSize: '.72rem', padding: '2px 8px' }} disabled={anularNota.isPending}
                            onClick={() => setConfirm({ id: n.id, tipo: 'nota', accion: 'anular' })}>Anular</button>
                        ) : (
                          <button className="btn btn-sm btn-danger" style={{ fontSize: '.72rem', padding: '2px 8px' }} disabled={eliminarNota.isPending} title="Eliminar permanentemente"
                            onClick={() => setConfirm({ id: n.id, tipo: 'nota', accion: 'eliminar' })}>🗑</button>
                        )}
                      </td>
                    </tr>
                  ))
